document.getElementById('launch-eet-btn').addEventListener('click', launchEet);
...
function addWebsite(e) {
    ...
    if(siteName.value === '') { 
        error = sendErrorMsg('Please enter the website\'s name.', 'alert-danger', siteName);
    }
    ...
    if(url.value === '' || url.value.search(regex) === -1) {
        ...
        url.value = url.value.split('//').length === 1 ? 'http://' + url.value : url.value;
    }
    ...
    for(const site of websites) {
        if(site.name === website.name || site.url === website.url) {
            if(site.name === website.name) {
                hasWebsite = sendErrorMsg('Website name already exists.', 'alert-warning', siteName);
            }
...
function showEditor() {
    ...
    if(urlFormGroup.className !== 'card-body') {
        ...
        currentSitesGroup.className = 'current-sites-rows row mt-3 text-center hidden';
